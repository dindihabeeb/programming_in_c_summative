# Programming in C Summative

This repo holds my summative projects for the low level / systems programming course. Each project sits in its own folder with its own README, source, and Makefile where it needs one.

## Projects

1. Smart Parking Lot Monitoring System (kept separately, added later)
2. Linux File Backup and Recovery Manager, a Bash tool for backing up and restoring folders. See `linux_file_backup_recovery_manager/`.
3. Library Book Inventory Management System, written in C with dynamic memory and file storage. See `library_book_inventory_management_system/`.
4. Unit Conversion Toolkit, a C program built around function pointers and callbacks. See `smart_calculator_conversion_toolkit/`.
5. Multi-threaded File Processing System, C with POSIX threads. See `multithreaded_file_processing_system/`.

## Building

The C projects each have a Makefile:

```bash
cd <project_folder>
make
```

The Bash project just needs to be made executable and run. Each folder's README has the details.
